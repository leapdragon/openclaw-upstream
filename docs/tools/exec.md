...
| `tools.exec.reviewer.maxTokens`      | `1024`                   | Completion token budget for one review, clamped to the reviewer model's advertised `maxTokens`.                                                                    |
...
Set `tools.exec.reviewer.maxTokens` to change the completion budget of one review (default `1024`). A reviewer that thinks before answering needs headroom for both the reasoning and the JSON verdict; if verdicts come back truncated, raise it, for example `reviewer: { model: "vllm/qwen3-8b", thinking: "low", maxTokens: 2048 }`. The value is clamped to the reviewer model's advertised `maxTokens`.
